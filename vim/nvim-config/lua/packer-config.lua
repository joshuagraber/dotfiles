    ...
    requires = { -- LSP Support
      {
        ...
        -- *** THE CRITICAL CONFIG HOOK ***
        config = function()
          -- This ensures your complex LSP config file is loaded ONLY when nvim-lspconfig
          -- and all its dependencies (like none-ls) are ready.
          local status_ok, lsp = pcall(require, "lsp")
          if not status_ok then
            -- Print a warning if the file still fails to load, but it won't crash Neovim.
            vim.notify("LSP configuration failed to load at startup.")
          end
        end,
      },
      -- Autocompletion
